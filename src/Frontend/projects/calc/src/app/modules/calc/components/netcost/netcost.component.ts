...
@Component({
    selector: 'company-netcost',
    templateUrl: './netcost.component.html',
    styleUrls: ['./netcost.component.scss'],
})
export class NetcostComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        ...
        this._calcService.setPaginatorParams(PaginatorType.Netcost);
        ...
        this._subscriptions.push(combineLatest(
            ...
            .pipe(
                tap(([brand, season, brands, seasons]) => {
    ...
    public handlePageEvent(event: PageEvent) {
        ...
        this._calcService.setPaginatorParams(PaginatorType.Netcost, this.pageNumber, this.pageSize);
        this.loadProducts();
    }
    ...
    public isExistLandingCost = (product: Product) => product.properties.some((p) => p.attribute.id === env.attributesIds.lc && !!p.attributeValue.numValue);
    ...
    private _openEditAttributesDialog(): MatDialogRef<NetcostEditAttributesDialogComponent> {
        return this._matDialog.open(NetcostEditAttributesDialogComponent, {
            data: {
